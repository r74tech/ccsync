...
describe("CcSync Integration Tests", () => {
	...
	describe("Full workflow", () => {
		it("should handle complete sync workflow", async () => {
			...
			await writeFile(
				join(sourceDir, "src", "components", "claude.md"),
				"# Components claude.md",
			);

			// Initialize ccsync
			const { stdout: initOutput } = await runCcsync(
				`init --destination ${join(testDir, "sync")}`,
			);
	...
	describe("Gitignore handling", () => {
		it("should respect .gitignore by default", async () => {
			...
			await expect(
				access(join(destDir, "ignored", "claude.md")),
			).rejects.toThrow();
		});
		...
		it("should include gitignored files when specified", async () => {
			...
			await runCcsync(`init --destination ${join(testDir, "sync")}`);
			await runCcsync(
				`add-project --name test --source ${sourceDir} --include-git-ignored`,
			);
			const { stdout } = await runCcsync("sync test");
	...
	describe("Custom destination", () => {
		it("should use custom destination when specified", async () => {
			...
			await runCcsync(`init --destination ${join(testDir, "sync")}`);
			await runCcsync(
				`add-project --name test --source ${sourceDir} --destination ${customDest}`,
			);
