...
describe("CLI", () => {
	...
	describe("init command", () => {
		it("should require destination option", async () => {
			await expect(cli(["node", "ccsync", "init"])).rejects.toThrow("process.exit called")
	...
	describe("add-project command", () => {
		it("should require name and source options", async () => {
			await expect(cli(["node", "ccsync", "add-project"])).rejects.toThrow("process.exit called")
		...
		it("should require source when only name is provided", async () => {
			await expect(cli(["node", "ccsync", "add-project", "--name", "test"])).rejects.toThrow(
				"process.exit called",
			)
	...
	describe("help command", () => {
		it("should show help when no command is provided", async () => {
			await expect(cli(["node", "ccsync"])).rejects.toThrow("process.exit called")
		...
		it("should show help with --help flag", async () => {
			await expect(cli(["node", "ccsync", "--help"])).rejects.toThrow("process.exit called")
	...
	describe("version command", () => {
		it.skip("should show version with --version flag", async () => {
			await expect(cli(["node", "ccsync", "--version"])).rejects.toThrow("process.exit called")
